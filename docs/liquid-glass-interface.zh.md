...
选择反馈使用一块持续存在的玻璃底板，在选项之间插值位置和尺寸。限制弹性过冲，避免底板从菜单左右边缘弹出；悬停反馈应明显弱于选中状态。
...
| 方法 | Demo 实现 |
| --- | --- |
| 环境与折射分层 | 应转为环境坐标对齐的应用可控场景副本、位移贴图和 SVG 滤镜定义 |
| 透明填充与边缘光学 | `app/globals.css` 中的玻璃填充、内高光和阴影 |
| 主题独立调校 | 深色 `liquid-lens-filter` 与亮色 `liquid-lens-filter-light` |
| 顶栏和菜单关系 | 菜单开合状态与耦合场动画 |
| 移动选择底板 | 根据当前选项位置移动的共享玻璃底板 |
| 可选拖动 | Pointer Events、指针捕获和展示区域边界限制 |
| 动效降级 | `prefers-reduced-motion` 和无滤镜回退样式 |

## 本轮盲测结论与可迁移核心

本轮盲测未通过：静态/结构与截图自动检查不足以证明真实折射、裁切边界和交互方向正确；页面还必须在隔离可运行页中由用户亲自体验。尤其不能把 `feTurbulence`、固定渐变伪元素、局部重复渐变或仅 `backdrop-filter` 称为增强折射。只有背景网格、文字或色带在玻璃边缘出现随位置变化的弯曲，才可称为增强折射。

原 Demo 仍可迁移的核心包括五层分层、亮暗独立调校、菜单与工具栏的状态耦合、共享选择底板、Pointer Events 拖动边界，以及减少动态和无滤镜降级。迁移时必须以实测二维方向、光学层隔离裁切和人工视觉门替代原有的固定方向、整组件裁切或自动检查即验收的做法。
...
当请求包含“接近原 Demo”“视觉保真”“9/10”“不要重新设计”或“复现”时，必须使用低自由度保真模式。复制 `skills/liquid-glass-interface/assets/fidelity-kit/`，不得自行改写共享 `SceneArtwork`、按实际几何生成的圆角 SDF 位移场、实例安全 RGB 滤镜、世界坐标对齐副本、亮暗材质参数、toolbar/popover 耦合或测量式选择底板。可修改文案、语义菜单项、布局与共享场景模型中的颜色。

`assets/reference-implementation/` 是技术基础示例，不是高保真模板。保真盲测必须在顶部、中部、底部三个滚动位置打开同一菜单，并使可读大字、网格和色带穿过镜片；可见场景与每个副本必须调用同一场景函数，每个表面必须按自己的宽、高和圆角生成位移场。用户未明确体验并通过时，盲测不通过；截图和自动检查只能作为辅助证据。

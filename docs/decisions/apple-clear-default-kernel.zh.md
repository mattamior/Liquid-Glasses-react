...
- 标签裁剪：`.apple-menu-visual--above` / `--below` 跟着旅行板，不跟着 idle 行。上沿为 `menu-pad + selection-y + travel-y-nudge`，下沿再加 `--apple-selection-height`。再减/加 `--apple-lens-clip-overshoot`（板高 × `(stretchY − 1) / 2`），盖住 `scaleY(1.16)` 从圆角探出的大字。idle 时 stretch 为 1，过冲为 0。透镜标签仍用 `translate3d(0, calc(-12px - var(--apple-selection-y) - var(--apple-travel-y-nudge)), 0)`。clip-path 不单独做动画。

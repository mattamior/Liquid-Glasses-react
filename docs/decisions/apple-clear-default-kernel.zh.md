...
- 标签裁剪：`.apple-menu-visual--above` / `--below` 只按 idle `58px` 条目轨道裁剪——`inset(0 0 calc(100% - (12px + var(--apple-selection-y))) 0)` 与 `inset(calc(12px + var(--apple-selection-y) + 58px) 0 0 0)`。**不**包含 `--apple-travel-y-nudge` 或 `--apple-selection-height`。58px 标签洞锁在条目上。选择板可高出条目 8px；相邻标签仍可见（液体重叠）。透镜标签用 `translate3d(0, calc(-12px - var(--apple-selection-y) - var(--apple-travel-y-nudge)), 0)`，字留在 58px 轨道中心。菜单文字层为 `inset: 0 8px auto 8px`、`grid-auto-rows: 58px`、`align-content: start`，避免 idle 行被底部剩余 padding 撑高（否则 idle 字形会比透镜低约 7.5px）。滑块消失时中心点不再位移。clip-path 不单独做动画。

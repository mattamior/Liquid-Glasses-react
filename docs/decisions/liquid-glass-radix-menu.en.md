...
The portable deliverable is the floating menu, not the home-screen lab. New work lives on `grok/liquid-glass-radix-menu`. Product mounts copy the kernel and render `LiquidMenu` or `LiquidDropdown`. Radix owns open, focus, and dismiss. Optics stay frozen.

This batch finishes **only** `LiquidDropdown`. Other overlays stay unfinished and are not patched here.
...
- `LiquidContextMenu`, `LiquidSelect`, `LiquidPopover`, `LiquidDialog`, and `LiquidMenubar` remain catalog stubs from the earlier 5-pack. They are not completed in this batch.
...
- Next batch: finish `LiquidContextMenu` only, reusing `host="nested"`.

...
- Label clip: `.apple-menu-visual--above` / `--below` clip to the idle `58px` item rail only — `inset(0 0 calc(100% - (12px + var(--apple-selection-y))) 0)` and `inset(calc(12px + var(--apple-selection-y) + 58px) 0 0 0)`. They do **not** include `--apple-travel-y-nudge` or `--apple-selection-height`. The 58px label hole stays locked to the item. The plate may plump 8px past the item; neighbor labels stay visible (liquid overlap). Lens labels use `translate3d(0, calc(-12px - var(--apple-selection-y) - var(--apple-travel-y-nudge)), 0)` so the active word stays centered on the 58px rail while the plate grows and shrinks. Menu visuals use `inset: 0 8px auto 8px`, `grid-auto-rows: 58px`, and `align-content: start` so idle rows are not stretched to the leftover bottom padding (that stretch made idle glyphs ~7.5px lower than the lens). Fade-out therefore does not slide the glyph center. Clip-path is not animated separately.
...
- Menu item, menu pane, frame, and selection plate (idle and travel) all use `--apple-menu-radius: 28px`. Travel no longer switches to `32px`. Selection SDF `radiusCssPx` is `28`.

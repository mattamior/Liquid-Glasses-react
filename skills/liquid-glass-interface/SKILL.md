...
2. For `apple-liquid-glass`, copy every file in [assets/strict-kernels/apple/](assets/strict-kernels/apple/) and the matching `apple-strict-adapter.tsx`. Mount `LiquidMenu` for an always-visible panel or `LiquidDropdown` for trigger-plus-portal. Do not edit optics, the traveling plate, or the spring. The layer immediately behind the menu must be blur or a solid color so labels stay readable. `HomeScreenScene` is lab chrome only (`variant: "lab"`); do not require it in product mounts.
